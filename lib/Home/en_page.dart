...
import 'package:game_app_tap/models/atasozleri.dart';
...
class _EnglishPageState extends State<EnglishPage> {
  ...
  void didChangeDependencies() {
    ...
    _bannerAd = BannerAd(
      size: AdSize.banner,
      adUnitId: 'ca-app-pub-2452889629536861/4771382970',
  ...
  FutureBuilder<List<dynamic>> buildFutureBuilder() {
    return FutureBuilder(
        ...
        builder: (context, sonuc) {
          ...
          if (sonuc.hasData) {
            allAtasozu = sonuc.data as List<Atasozu>?;
            return GridView.builder(
              ...
              itemBuilder: (context, index) {
                return InkWell(
                  onTap: () {},
                  child: Hero(
                    tag: "${allAtasozu![index].en}",
                    child: GestureDetector(
                      onLongPress: () async {
                        await player
                            .setAsset(allAtasozu![index].soundSrc.toString());
                        player.play();
                      },
                      child: Column(
  ...
  Future<List> veriKaynaginiOku() async {
    var gelenJson = await DefaultAssetBundle.of(context)
        .loadString('assets/data/atasozleri.json');

    List<Atasozu> hayvanListesi = (json.decode(gelenJson) as List)
        .map((mapYapisi) => Atasozu.fromJson(mapYapisi))
        .toList();
